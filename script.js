...
async function preencherDropdown(parametros) {
    if (parametros.tipoIntercambio && parametros.cl && parametros.anuncio) {
        ...
        console.log(campos.find(field => field.label === "Qual é a AIESEC mais próxima de você?").config.settings.options.filter(opcoes => opcoes.status == "active").filter((_, index) => index === indiceSiglaCL))
